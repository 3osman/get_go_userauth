...
+ function($) {
	...
	OffCanvas.prototype.show = function() {
		if (this.state)
			return $(".navbar-toggle").removeClass("hamburger");
		...
		if (elements.index(this.$element) !== -1) {
			...
			// Workaround: Need to get the CSS property for it to be applied before the next line of code
		}
	...
	OffCanvas.prototype.hide = function(fast) {
		if (this.state !== 'slid')
			return $(".navbar-toggle").addClass("hamburger");

		var startEvent = $.Event('hide.bs.offcanvas')
		this.$element.trigger(startEvent)
		if (startEvent.isDefaultPrevented())
			return this.state = 'slide-out'
	...
	OffCanvas.prototype.toggle = function() {
		if (this.state === 'slide-in' || this.state === 'slide-out')
			return this[this.state === 'slid' ? 'hide' : 'show']()
	}
...
+ function($) {
	...
	Inputmask.prototype.shiftR = function(pos) {
		...
		for (var i = pos,
		    c = this.options.placeholder; i < len; i++) {
			if (this.tests[i]) {
